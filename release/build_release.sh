...
cd $BUILD_DIR
git init
git remote add origin https://github.com/NagasPilot/openpilot.git
...
git add -f .
git commit -a -m "NagasPilot v$VERSION release"
...
git add -f .
git commit --amend -m "NagasPilot v$VERSION"
