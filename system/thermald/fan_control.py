...
def _build_default_bands() -> OrderedDict[ThermalStatus, ThermalBand]:
  """Build default fan bands from the closed HAL, with a hardcoded fallback."""
  if _FAN_BANDS:
    mapping = {
      "GREEN": ThermalStatus.GREEN,
      "YELLOW": ThermalStatus.YELLOW,
      "RED": ThermalStatus.RED,
      "DANGER": ThermalStatus.DANGER,
    }
    return OrderedDict(
      (mapping[name], ThermalBand(
        b["min_temp"], b["max_temp"], b["min_fan"], b["max_fan"]
      ))
      for name, b in _FAN_BANDS.items()
      if name in mapping
    )
  return OrderedDict({
...
DEFAULT_BANDS = _build_default_bands()
...
class FanController:
    ...
    def __init__(self, bands: OrderedDict[ThermalStatus, ThermalBand] | None = None, temp_tau: float = 5.0, min_fan_speed: int = 0):
    ...
    def _smooth_temperature(self, new_temp: float, dt: float) -> float:
        ...
        """
        if self._smoothed_temp == 0.0 or dt <= 0.0 or self.temp_tau <= 0.0:
            return new_temp
