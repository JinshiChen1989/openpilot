...
class ThermalProtectionConfig:
    ...
    warning_temp: float = _THERMAL_DEFAULTS.get("warning_temp", 75.0)
    throttle_temp: float = _THERMAL_DEFAULTS.get("throttle_temp", 85.0)
    critical_temp: float = _THERMAL_DEFAULTS.get("critical_temp", 105.0)
    hysteresis: float = _THERMAL_DEFAULTS.get("hysteresis", 5.0)
...
class ThermalD:
    ...
    def __init__(self):
        ...
        # Zone failure tracking: tolerate transient sensor read errors,
        # but warn when a zone is persistently unreadable.
        self._zone_fail_counts: dict[str, int] = {}
        self._ZONE_FAIL_THRESHOLD = 5
    ...
    def _read_zone(self, zone: ThermalZone) -> float | None:
        temp = zone.read()
        if temp is None:
            fails = self._zone_fail_counts.get(zone.name, 0) + 1
            self._zone_fail_counts[zone.name] = fails
            if fails == self._ZONE_FAIL_THRESHOLD:
                cloudlog.warning(f"thermald: {zone.name} thermal zone read failed {fails} times")
            elif fails < self._ZONE_FAIL_THRESHOLD:
                cloudlog.debug(f"thermald: Failed to read {zone.name}")
        else:
            if self._zone_fail_counts.get(zone.name, 0) >= self._ZONE_FAIL_THRESHOLD:
                cloudlog.info(f"thermald: {zone.name} thermal zone read recovered")
            self._zone_fail_counts[zone.name] = 0
        return temp
