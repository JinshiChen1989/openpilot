...
def _flags_for(hardware_env: str | None) -> dict[str, bool]:
  ...
  keys = ['RK3588', 'ROCKCHIP', 'TICI', 'PC']
  return dict(zip(keys, (v == 'True' for v in values), strict=True))


def test_rockchip_and_tici_are_true_on_rk3588():
  assert _flags_for('rk3588') == {'RK3588': True, 'ROCKCHIP': True,
                                  'TICI': True, 'PC': False}


def test_pc_fallback_when_no_hardware_env():
  assert _flags_for(None) == {'RK3588': False, 'ROCKCHIP': False,
                              'TICI': False, 'PC': True}


class TestBranchHardwareScope:
  """This branch supports ExoPilot 01M (RK3588) hardware only.

  02M lives on dev/02M -- see the branch model in CLAUDE.md. These tests are
  the guard against 02M support drifting back in through a rebase or a
  well-meaning import, which would quietly recreate the coupling the branch
  ...
  def test_rk3576_is_not_a_known_platform(self):
    ...
    assert "rk3588" in registered
    assert "rk3576" not in registered

  def test_asking_for_rk3576_fails_loudly(self):
    # Not silently falling back to pc or rk3588: a board this branch cannot
    # drive must refuse rather than mislabel itself.
    result = _run("from openpilot.system.hardware import HARDWARE\nprint(HARDWARE)", 'rk3576')
    assert result.returncode != 0
    assert "Unknown platform: rk3576" in result.stderr

  def test_the_rk3576_package_is_gone(self):
    result = _run("import openpilot.system.hardware.rk3576.hardware", None)
    assert result.returncode != 0
  ...
  def test_the_board_independent_base_survives(self):
    ...
    RK3588Hardware because there is only one board today would undo that.
    """
    ...
    assert RK3588Hardware is not RockchipHardware
  ...
  def test_rk3588_still_owns_what_is_board_specific(self):
    from openpilot.system.hardware.rk3588.hardware import RK3588Hardware
    # Modem power control is the real electrical difference between boards:
    # 01M switches a Mini-PCIe USB-mode mux.
  ...
  def test_the_camera_array_is_01m_shaped(self):
    from openpilot.system.hardware.rk3588.hardware import RK3588Hardware
    assert RK3588Hardware.MIPI_CAMERA_NAMES == ("road", "wide_road",
                                                "stereo_left", "stereo_right")
    assert not RK3588Hardware.HAS_TELE_ROAD    # 01M has no telephoto
