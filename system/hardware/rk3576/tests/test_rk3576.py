...
def test_shares_camera_array_logic_with_the_rockchip_base() -> None:
  """RK3576Hardware must not reimplement get_camera_array_config()/
  get_stereo_baseline_mm() — it inherits them from RockchipHardware and only
  overrides the class-level platform/geometry attributes.

  These used to be inherited from RK3588Hardware, which is what made 01M's
  class load-bearing for 02M. The base is what broke that tie."""
  from openpilot.system.hardware.rockchip_base import RockchipHardware
  assert RK3576Hardware.get_camera_array_config is RockchipHardware.get_camera_array_config
  assert RK3576Hardware.get_stereo_baseline_mm is RockchipHardware.get_stereo_baseline_mm
...
def test_registry() -> None:
  """RK3576 is the only Rockchip board registered on this branch."""
  assert "rk3576" in PlatformRegistry._platforms
  assert "rk3588" not in PlatformRegistry._platforms
