...
it with `selfdrive/ui`, a Qt Widgets UI in Python).

What changed:

- `lupdate` scanned `.cc`/`.h` for `tr(...)`. The equivalent for a Python Qt
  UI is `pylupdate5` (or `pyside2-lupdate`) over `selfdrive/ui/**/*.py`.
...
`selfdrive/ui` are English literals, and wiring `QTranslator` plus
