...
class PatternDetector:
    ...
    def _detect_chessboard(self, gray: np.ndarray, image: np.ndarray) -> tuple[np.ndarray, np.ndarray] | None:
    ...
    def _detect_charuco(self, gray: np.ndarray) -> tuple[np.ndarray, np.ndarray] | None:
    ...
    def _detect_circles(self, gray: np.ndarray) -> tuple[np.ndarray, np.ndarray] | None:
...
def calibrate_camera(
    ...
    if len(img_points) < 5:
        print("Error: Need at least 5 images for calibration")
        return None
